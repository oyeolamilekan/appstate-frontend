...
export function DeveloperSideBar({ children }: PropTypes) {
  ...
  const { value } = useSessionStorage<{ name: string, public_id: string }>(CURRENT_STORE);
      ...
      <div className={`flex flex-shrink-0 flex-col justify-between md:sticky fixed inset-y-0 left-0 transform bg-slate-100 dark:bg-neutral-900 ${sideBarState ? '-translate-x-full' : ''} md:translate-x-0 transition duration-200 ease-in-out top-0 h-screen z-10`} ref={sideBarNode}>
        <div className='flex flex-col w-56'>
          <div className='py-5 px-4 mt-3'>
            <div className='mt-3 px-4 py-2 mb-4 dark:text-white hover:bg-gray-200 dark:hover:bg-neutral-800 rounded cursor-pointer'>
              <h3 className='text-lg font-semibold'>{value?.name}</h3>
              <span className='text-xs'>Merchant ID: {value?.public_id}</span>
            </div>
            <ul className='flex flex-col gap-3'>
              <li>
                <Link href={`/dashboard/home`} className={`flex flex-row gap-2 w-full dark:text-white items-center hover:bg-gray-200 dark:hover:bg-neutral-800 ${checkIfActive("/dashboard/home") ? "bg-gray-200 dark:bg-neutral-800 font-semibold" : ""} hover:font-semibold rounded px-3 py-2 transition-all duration-200`}>
                  <HomeIcon strokeWidth={checkIfActive("/dashboard/home") ? 2 : 1.5} />
                  <span className="text-[0.9rem]">Home</span>
                </Link>
              </li>
              <li>
                <Link href={`/dashboard/products`} className={`flex flex-row gap-2 w-full dark:text-white items-center hover:bg-gray-200 dark:hover:bg-neutral-800 ${checkIfActive("/dashboard/products") ? "bg-gray-200 dark:bg-neutral-800 font-semibold" : ""} hover:font-semibold rounded px-3 py-2 transition-all duration-200`}>
                  <ShoppingBag strokeWidth={checkIfActive("/dashboard/products") ? 2 : 1.5} />
                  <span className="text-[0.9rem]">Products</span>
                </Link>
              </li>
              <li>
                <Link href={`/dashboard/integrations`} className={`flex flex-row gap-2 w-full dark:text-white items-center hover:bg-gray-200 dark:hover:bg-neutral-800 ${checkIfActive("/dashboard/integrations") ? "bg-gray-200 dark:bg-neutral-800 font-semibold" : ""} hover:font-semibold rounded px-3 py-2 transition-all duration-200`}>
